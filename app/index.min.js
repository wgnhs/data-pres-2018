(function (global, factory) {
  ...
  class CheckboxControl {
    ...
    get next() {
      return litElement.html`<toggle-switch style="justify-content:start"></toggle-switch>`;
    }
    handle(context) {
      let result = null;
  ...
  class GTLTControl {
    ...
    get next() {
      return litElement.html`<select><option value="gt">${this.gtName}</option><option value="eq">${this.eqName}</option><option value="lt">${this.ltName}</option></select> <input type="text">`;
    }
    handle(context) {
      let result = null;
      context['gt'] = (a, b) => (a >= b);
  ...
  class SelectControl {
    ...
    get next() {
      return litElement.html`<select ?disabled="${!this.options}"><option></option>${(!this.options)?'':this.options.map((el) => litElement.html`<option value="${el}">${el}</option>`)}</select>`;
    }
    init(lookup) {
  ...
  class TextControl {
    ...
    get next() {
      return litElement.html`<input type="text">`;
    }
    handle(context) {
      let result = null;
  ...
  class TableLayout extends litElement.LitElement {
    ...
    static get styles() {
      return litElement.css`[data-element=table]{display:grid;grid-template-columns:40% 1fr;grid-gap:var(--border-radius);margin:0 var(--border-radius)}.label{font-weight:var(--font-weight-bold)}`;
    }
    ...
    render() {
      ...
      entries = entries.map((el, index) => litElement.html`<dt class="label" title="${SiteData.getFieldDescription(el[key], this.context.layoutName)}"><label for="${this.genId(index)}">${SiteData.getFieldTitle(el[key], this.context.layoutName)}</label></dt><dd class="detail" title="${SiteData.getFieldDescription(el[key], this.context.layoutName)}"><span id="${this.genId(index)}">${el[value]}</span></dd>`);
      return litElement.html`<dl data-element="table">${entries}</dl>`;
    }
  }
  ...
  class DownloadButton extends litElement.LitElement {
    ...
    static get styles() {
      return [
        ...wgnhsCommon.styles,
        litElement.css`[data-closed]{display:none}.file-size{font-size:var(--font-size-small)}`];
    }

    render() {
      return litElement.html`<button-link href="${this.src}" target="_blank" download ?data-closed="${!this.exists}"><i slot="content-before" class="material-icons" title="Download">save_alt</i> <span slot="content"><slot>Download</slot></span><span slot="content-after" class="file-size">${this.fileSize}</span></button-link>`;
    }
  ...
  class QuatCoreLayout extends litElement.LitElement {
    ...
    static get styles() {
     ...
     render() {
      return litElement.html`<table-layout .info="${this.prepInfo()}" .context="${this.context}"></table-layout><pdf-view-button .panel="${this.context.pdfpanel}" src="${'https://data.wgnhs.wisc.edu/quat-core-photos/' + this.info.WGNHS_ID + '.pdf'}"><span slot="download-text">Download Photos</span></pdf-view-button>`;
    }
  ...
  class RockCoreLayout extends litElement.LitElement {
    ...
    render() {
      return litElement.html`<table-layout .info="${this.prepInfo()}" .context="${this.context}"></table-layout><pdf-view-button .panel="${this.context.pdfpanel}" src="${'https://data.wgnhs.wisc.edu/core-photos/' + this.info.WID + '.pdf'}"><span slot="download-text">Download Photos</span></pdf-view-button>`;
    }
  ...
  class SiteDetails extends litElement.LitElement {
    ...
    static get styles() {
      return [
        ...wgnhsStyles.styles,
        litElement.css`.header{position:-webkit-sticky;position:sticky;top:0;background-color:var(--palette-white);padding:var(--font-size-extra-large) var(--border-radius);z-index:10;width:100%;box-sizing:border-box;display:flex;justify-content:space-between}.header h1{padding:0;max-width:70%;text-align:center}.header i{font-size:var(--icon-size-large);color:var(--palette-accent);cursor:pointer}.name{text-transform:capitalize}[data-closed]{display:none}`];
    }
    ...
    render() {
      ...
      return litElement.html`${(!this.siteinfo)? '' : litElement.html`<div class="header"><span><a href="${window.router.link('entry')}" onclick="event.preventDefault()"><i class="material-icons clear-selection" title="Clear selection" @click="${this.fireClearSelection}">arrow_back</i></a></span><h1>${this.siteinfo.Site_Name}</h1><span><i class="material-icons zoom-to-site" title="Zoom to site" @click="${this.fireZoomToSite}">my_location</i></span></div>${this.renderData({
          Latitude, Longitude, WID
        })} ${this.siteinfo.datas.slice().reverse().map((props) => litElement.html`<app-collapsible open><span slot="header" class="name">${props['Data_Type']}</span><div slot="content">${this.renderData(props, props['Data_Type'])}</div></app-collapsible>`)}`}`;
    }
  ...
  class FilterSummary extends litElement.LitElement {
    ...
    static get styles() {
      return litElement.css`li[disabled]{color:var(--palette-dark)}.name{text-transform:capitalize}`;
    }

    render() {
      return (!this.counts)?litElement.html``:litElement.html`<div><span>Showing ${this.counts.reduce((prev, count) => (count.current + prev), 0)} of ${this.counts.reduce((prev, count) => (count.total + prev), 0)} total sites</span><ul>${this.counts.reduceRight((prev, el) => prev.concat(litElement.html`<li ?disabled="${!el.included}">${el.current} of ${el.total} <span class="name">${el.name}</span> sites ${(el.filteredBy.length > 0)?litElement.html`(filtered by ${el.filteredBy.join(', ')})`:''}</li>`),[])}</ul></div>`;
    }
  ...
  class MapFilter extends litElement.LitElement {
    ...
    static get styles() {
      return [
        ...wgnhsStyles.styles,
        litElement.css`.field{display:grid;grid-template-columns:40% 1fr;grid-gap:var(--border-radius);margin:0 var(--border-radius)}.label{color:var(--palette-900);font-weight:var(--font-weight-bold)}.section-title{margin:var(--line-height) 0 0 0;padding:var(--border-radius);background-color:var(--palette-light)}[data-closed]{display:none}in-radio{display:inline-grid;grid-template-columns:auto auto}.collapse-icon::before{content:"expand_more"}[open]>.collapse-icon::before{content:"expand_less"}.raw-source-button{display:flex;margin-top:var(--line-height)}.raw-source-button>*{font-size:var(--font-size)}`];
    }
    ...
    render() {
      return litElement.html`<div data-closed>Show sites that have<in-radio choices='["ALL", "ANY"]' @choice-change="${this.updateMatchClass}"></in-radio>of the following:</div><div>${this.renderFilterGroups()}</div>`;
    }

    renderFilterGroups() {
      return this.filterGroups.map((group, index) => litElement.html`<slot name="${index}"></slot><app-collapsible class="group" ?open="${group.open}"><i slot="header-before" class="material-icons collapse-icon"></i> <span slot="header">${group.title}</span> ${(!group.toggleable)?'':litElement.html`<toggle-switch name="${group.title}" slot="header-after" ?checked="${group.active}" @change="${this._handleGroup(group, 'include')}" style="--palette-active:${group.color}"></toggle-switch>`}<div slot="content">${group.sections.map((section, index) => litElement.html`${!(section.title)?'':litElement.html`<h2 class="section-title">${section.title}</h2>`} ${Object.entries(section.fields).map(([name, config], index) => (!config.controls)?'':litElement.html`<div class="field">${(config.controls.length === 0)?'':config.controls.map(control => litElement.html`<td class="label"><label for="${this.genId(index)}">${config.title}</label></td><td class="selector" @change="${this._handleControl(group, control, 'filter')}"><input type="hidden" id="${control.id}" name="${config.fieldKey}"> ${control.next}</td>`)}</div>`)}`)} ${(group.source && group.source.user)?litElement.html`<button-link href="${group.source.user}" class="raw-source-button"><span slot="content">View data source</span> <i slot="content-after" class="material-icons">open_in_new</i></button-link>`:''}</div></app-collapsible>`);
    }
